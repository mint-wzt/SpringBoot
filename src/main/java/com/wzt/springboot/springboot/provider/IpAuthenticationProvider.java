package com.wzt.springboot.springboot.provider;
...
public class IpAuthenticationProvider implements AuthenticationProvider {
    ...
    public Authentication authenticate(Authentication authentication) throws AuthenticationException {
        ...
        SimpleGrantedAuthority grantedAuthority = ipAuthorityMap.get(ip);
        //不在白名单列表中
        if (grantedAuthority == null) {
            return null;
        } else {
            //封装权限信息，并且此时身份已经被认证
            return new IpAuthenticationToken(ip, Arrays.asList(grantedAuthority));
        }
